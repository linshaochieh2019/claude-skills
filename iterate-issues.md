...
**Scan.** Walk the batch's commits (`git log origin/$BASE_BRANCH..HEAD --stat`) and ask, per fix: does the root cause belong to a class a machine can catch? A candidate must meet **all three**:

1. **Mechanical, not semantic.** Detectable by pattern — banned API/import (use `no-restricted-imports` / `no-restricted-syntax` or the stack's equivalent), naming/file-placement convention, a required pairing (e.g. new endpoint ⇒ authz check), a "never modify existing migrations" invariant. Judgment calls ("bad abstraction", "wrong layer") are NOT candidates — those stay in CLAUDE.md and review. A required-pairing rule qualifies **only if the pairing is unconditional**: if legitimate exceptions exist (an intentionally ungated route, an intentionally exempt tool), the exception set is itself a semantic judgment — keep it in review, not the linter.
...
3. **Not already enforced.** Skip any class already caught by an existing gate — a committed test, the typechecker, or a CI-wired script — **including a guard added earlier in this same batch**. If the batch's own fix was to add or extend a guard, the class is already permanent; do not wrap it twice. (Dry-run evidence: in a typical batch, most mechanical fixes are self-guarding — this disqualifier is what keeps the pass from rule-spamming.)

**No candidates → skip silently and go to 5.2.** That's the common case; do not force a rule out of every batch.

**Dispatch.** For candidates (cap: **2 rules per batch** — precision over recall, rule-spam erodes trust in the linter), dispatch ONE Sonnet subagent, cwd `$WORKTREE_DIR`, with: the class description, the offending pattern + corrected pattern from the batch's own commits, and these directives — prefer the repo's **existing mechanical-enforcement layer**, whatever it is (linter config if present; otherwise the test suite or a CI-wired script, e.g. `scripts/*.mjs` — never scaffold a new linter into a repo that doesn't have one); the rule's error message must state the correct alternative (that message is what steers the next agent); the full verification suite AND the new rule must pass on the current branch tip; commit as `hardening: <what it prevents> (class from #<N>)`.
